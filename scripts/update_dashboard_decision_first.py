...
text = text.replace('Totals are currently the strongest validated research signal.', 'Totals are currently the strongest early research-supported signal.')
text = text.replace('strongest validated research signal', 'strongest current research signal')
...
new = '<div class="metric decision"><div class="k">Market / Consensus</div><div class="v">${g.market_bookmaker||"Market"} ${marketSpread(g)} · ${N(g.market_total)}</div><div class="note">Consensus spread ${E(g.consensus_home_spread)?(Number(g.consensus_home_spread)>0?"+":"")+N(g.consensus_home_spread):"—"} · total ${N(g.consensus_total)} · ${g.market_book_count||1} books</div><div class="note ${marketFresh?\'fresh\':\'stale\'}">${marketFresh?\'Fresh\':\'⚠ Stale\'} · ${ago(g.market_updated_at)}</div></div>'
text = text.replace(old,new)

path.write_text(text, encoding="utf-8")
print("Updated dashboard with independent model, consensus market ranges, and cautious research language")
